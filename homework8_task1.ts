...
const get_title = book_item => book_item[0];

// Retrieves the ISBN of a book.
const get_isbn = book_item => book_item[1];

// Returns true if the given book is marked as checked out.
const is_checked_out = book_item => book_item[2];

// Returns a book with the same title and ISBN, but marked as checked out.
const check_out_book =
    book_item => [get_title(book_item), get_isbn(book_item), true];

// Returns a book with the same title and ISBN, but marked as checked in.
const check_in_book =
    book_item => [get_title(book_item), get_isbn(book_item), false];

// An empty Library.
const empty_library = list();
...
function add_book(book, library) {
    ...
    function insert(lib) {
        ...
        return is_null(lib)
               ? list(book)
               : get_isbn(head(lib)) < isbn_book
               ? pair(head(lib), insert(tail(lib)))
               : pair(book, lib);
    }
